...
export default function ExplorePage() {
  ...
  const allTopics = useMemo(
    () =>
      Array.from(
        new Set(
          CATEGORIES.filter((c) => c.id !== "all").flatMap(
            (c) => c.topics || []
          )
        )
      ),
  ...
  const shouldShowMoreButton =
    filteredCommunities.length > limit || hasMore || page > 1;
  ...
  const handleJoinCommunity = async (e, community) => {
    ...
    setJoiningCommunity(community.name);
    try {
      if (community.isJoined) {
        // Leave the community
        await api.post(`/communities/${community.name}/leave`);
        ...
        // Join the community
        await api.post(`/communities/${community.name}/join`);
        setCommunities((prevCommunities) =>
          prevCommunities.map((c) =>
            c.name === community.name
              ? {
      ...
      if (err.response?.status === 401) {
        ...
        alert(err.response?.data?.message || "Failed to update community");
      }
    } finally {
        ...
        <div className="explore-container">
          <section className="communities-section">
            ...
            {!loading && filteredCommunities.length === 0 && (
              <p className="no-communities-message">
              ...
              {filteredCommunities.map((community) => (
                ...
                >
                  <div className="community-header">
                    ...
                    >
                      {joiningCommunity === community.name
                        ? community.isJoined
                          ? "Leaving..."
                          : "Joining..."
                        : community.isJoined
            ...
            {shouldShowMoreButton && (
              <button
                ...
                disabled={loading || (!hasMore && page === 1)}
              >
                {loading
                  ? "Loading..."
                  : page > 1 && !hasMore
